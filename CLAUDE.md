...
- **Single remote.** Unlike upstream's dual public/private org setup, this is a
  single private repo. Day-to-day work and releases happen on `main` (or feature
  branches), gated by normal PR review.
- **Self-contained install.** `install.ps1` / `install.sh` install from this
  repo directly — no clone of upstream at install time.
- **Syncing upstream.** To pull upstream improvements, diff a fresh
  `AgriciDaniel/claude-seo` checkout against `skills/` and `agents/` (normalize
  line endings first), re-apply the `seo-audit/SKILL.md` patch (Evidence
  Integrity Protocol + fetch-path fix), and keep `onboarding/` and
  `skills/seo-setup/` intact.
- **New surface area to maintain:** `onboarding/` (the wizard) and
  `skills/seo-setup/` (the `/seo-setup` skill). Everything else mirrors upstream.
